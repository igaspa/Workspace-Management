...
const WorkspaceForm = ({ workspace, onSave, onCancel }) => {
  ...
  const calculateAddedAndRemovedItemLists = (formData) => {
    ...
    const addedAccessories = currentEquipment.filter(
      (newItem) => !oldWorkspaceEquipment.some((oldItem) => oldItem.id === newItem.id)
    );

    const removedAccessories = oldWorkspaceEquipment.filter(
      (newItem) => !currentEquipment.some((oldItem) => oldItem.id === newItem.id)
    );
                  ...
                  {selected.map((currEq) => {
                    ...
                    return (
                      <Chip
                        ...
                        onDelete={(event) => handleDelete(eq.id)}
