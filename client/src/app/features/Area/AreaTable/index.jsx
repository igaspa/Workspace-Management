...
export default function AreaTable() {
  ...
  const {
    data: [searchArea, searchAreaPages] = [],
  ...
  const filteredData =
    searchTerm.length >= 3
      ? searchArea.filter((item) => item.name.toLowerCase().includes(searchTerm.toLowerCase()))
      : area;
        ...
        {isAreasLoading || isLocationsLoading || isAreaSearchLoading ? (
          ...
          <>
            <Grid component="form" ref={searchRef} container alignItems="center" paddingBottom={1}>
              <SearchField data={searchArea.map((item) => item.name)} name="area" onChange={handleSearchTerm} />
