...
} from '../../../api/workspaceTypeApiSlice';
...
export default function WorkspaceTypeTable() {
  ...
  const {
    data: [searchWorkspaceType, searchWorkspaceTypePages] = [],
    isError: isWorkspaceTypeSearchError,
    isLoading: isWorkspaceTypeSearchLoading
  } = useGetWorkspaceTypeSearchListQuery({
    name: [searchTerm],
    ...(page && { page: page + 1 }),
    ...(size && { size })
  ...
  const filteredData =
    searchTerm.length >= 3
      ? searchWorkspaceType.filter((item) => item.name.toLowerCase().includes(searchTerm.toLowerCase()))
      : workspaceType;
  ...
  const data = filteredData?.map((el) => ({
    ...
    maxReservationInterval: `${
      el.maxReservationInterval.hours ? el.maxReservationInterval.hours.toString().padStart(2, '0') : '00'
    }:${el.maxReservationInterval.minutes ? el.maxReservationInterval.minutes.toString().padStart(2, '0') : '00'}`,
