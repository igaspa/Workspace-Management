...
exports.workspace_collection = joi
  .object({
    permanentlyReserved: joi.boolean().optional(),
    typeId: joi.string().guid({ version: 'uuidv4' }).required(),
    areaId: joi.string().guid({ version: 'uuidv4' }).required(),
    participantLimit: joi
      .number()
      .min(1)
      .max(100)
      .alter({
    ...
    prefix: joi.string().min(2).max(30).required(),
    start: joi.number().min(1).required(),
    end: joi.number().min(1).required(),
    addedAccessories: joi.array().items(
      joi.object().keys({
        id: joi.string().guid(),
        quantity: joi.number()
      })
...
// Reservation Entity Schema
exports.reservation_permanent = joi
  .object({
    id: joi
      .string()
      .guid({ version: 'uuidv4' })
...
exports.user_password_create = joi
  .object({
    password: joi
      .string()
      .min(5)
      .alter({
        ...
        put: (userSchema) => userSchema.required()
      })
  })
  .options({ abortEarly: false });
...
exports.workspace_equipment = joi
  .object({
    workspaceId: joi
      .string()
      .guid({ version: 'uuidv4' })
