...
export default function LocationTable() {
  ...
  const {
    data: [searchLocation, searchLocationPages] = [],
  ...
  const filteredData =
    searchTerm.length >= 3
      ? searchLocation.filter((item) => item.address.toLowerCase().includes(searchTerm.toLowerCase()))
      : location;
        ...
        isLocationsLoading || isLocationSearchLoading ? (
          ...
          <>
            <Grid component="form" ref={searchRef} container alignItems="center" paddingBottom={1}>
              <SearchField data={searchLocation.map((item) => item.name)} name="location" onChange={handleSearchTerm} />
